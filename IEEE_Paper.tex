...
At runtime, the smartphone client first performs image quality checks (focus and framing), then applies zone extraction from annotated card geometry. Each zone is resized to $224\times224$ and normalized before MobileNetV2 inference. The three zone outputs are transformed into binary reactions and mapped by deterministic ABO/Rh rules. This hybrid design keeps medical decision logic interpretable while retaining deep-model adaptability for visual pattern recognition.
...
\begin{figure}[htbp]
  ...
  \caption{Qualitative inference example from the project supplementary demo artifact (\url{https://github.com/sayonmitra-tech/AI_Blood_Group_detection}), showing zone-wise interpretation and final ABO/Rh decision generation.}
  \label{fig:ai_output}
\end{figure}
...
The codebase is implemented in Python with TensorFlow/Keras, OpenCV, NumPy, and Matplotlib. Model training and evaluation scripts are provided in the public project repository (\url{https://github.com/sayonmitra-tech/AI_Blood_Group_detection}), including the end-to-end pipeline (\texttt{blood\_group\_detection.py}) and evaluation workflow (\texttt{evaluation.py}). Exact experiment-runtime metadata (GPU model, TensorFlow build number, CUDA/cuDNN versions, and wall-clock training duration) are missing from the archived run logs for this release; future revisions will include these fields via explicit experiment tracking exports.
