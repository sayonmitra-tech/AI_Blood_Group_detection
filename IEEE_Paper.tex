...
Email: sayon@sayonedu.in, sayonmitra84@gmail.com}}

\maketitle

\begin{abstract}
Blood group identification is a prerequisite for safe transfusion, yet manual haemagglutination interpretation remains operator-dependent and difficult to standardize outside laboratory settings. This paper presents a research-stage assistive AI diagnostic framework for card-based ABO and Rh(D) classification from camera images. The pipeline combines COCO annotation-guided region extraction, OpenCV preprocessing, and MobileNetV2 transfer learning to classify clumping in Anti-A, Anti-B, and Anti-D zones, followed by deterministic ABO+Rh decision logic. Experiments on 7,836 images (6,456 train, 997 validation, 383 test) show approximately 94--95\% zone-level accuracy, 91--93\% end-to-end ABO accuracy, and macro F1-score of 0.947 at zone level. The MobileNetV2 backbone supports TensorFlow Lite conversion and edge inference on smartphones, making the framework suitable for assistive point-of-care screening in resource-constrained environments while requiring laboratory confirmation through certified transfusion workflows.
\end{abstract}
...
\subsection{Mobile AI Inference Pipeline}
At runtime, the smartphone client first performs image quality checks (focus and framing), then applies zone extraction from annotated card geometry. In the current release, these checks are heuristic UI-level gates. Their exact numeric thresholds were not version-logged in archived notebooks, so downstream users may need deployment-specific threshold tuning. If quality checks fail, the interface requests image recapture and suppresses automated blood-group output until a usable frame is obtained. Each zone is resized to $224\times224$ and normalized before MobileNetV2 inference. The three zone outputs are transformed into binary reactions and mapped by deterministic ABO/Rh rules. This hybrid design keeps medical decision logic interpretable while retaining deep-model adaptability for visual pattern recognition.
...
Supplementary qualitative outputs used in this analysis are provided in the public repository (\url{https://github.com/sayonmitra-tech/AI_Blood_Group_detection}).

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.95\columnwidth]{assets/figures/sample_blood_card.jpg}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.95\columnwidth]{assets/figures/demo_inference_output.jpg}
  \caption{Qualitative inference example from supplementary demonstration materials, showing zone-wise interpretation and final ABO/Rh decision generation.}
  \label{fig:ai_output}
\end{figure}
...
The codebase is implemented in Python with TensorFlow/Keras, OpenCV, NumPy, and Matplotlib. Model training and evaluation scripts are publicly available at \url{https://github.com/sayonmitra-tech/AI_Blood_Group_detection}, including the end-to-end pipeline (\texttt{blood\_group\_detection.py}) and evaluation workflow (\texttt{evaluation.py}). Exact experiment-runtime metadata (GPU model, TensorFlow build number, CUDA/cuDNN versions, and wall-clock training duration) are missing from archived run logs in the current release. Future revisions will include these fields via explicit experiment-tracking exports.

\section{Reproducibility}
To support reproducible reporting, dataset splits, model architecture, and inference rules are documented in this manuscript and in the repository implementation. Available experiment metadata include:
\begin{itemize}
  \item Dataset split: \textbf{6,456 train / 997 validation / 383 test} (total 7,836)
  \item Input resolution: \textbf{$224\times224$}
  \item Batch size: \textbf{32}
  \item Initial learning rate: \textbf{$1\times10^{-4}$}
  \item Max epochs: \textbf{10} with early stopping and LR reduction
\end{itemize}
GPU type, TensorFlow runtime version, CUDA/cuDNN versions, and end-to-end training time were not persistently logged in the current archived run history. Future releases will include full experiment manifests and environment capture for stricter reproducibility.
Package-level version pins (e.g., TensorFlow/OpenCV/NumPy) are also unavailable from the archived runtime export and will be included in future reproducibility manifests.

\section{Real-World Deployment Considerations}
\begin{itemize}
  \item \textbf{Human oversight:} Predictions should be reviewed by trained personnel before any transfusion-related decision.
  \item \textbf{Acquisition protocol:} Standardized camera distance, card orientation, and illumination are needed to reduce domain shift.
  \item \textbf{Quality control:} Low-confidence or low-quality images should trigger retesting rather than automated acceptance.
  \item \textbf{Regulatory pathway:} Any clinical deployment requires prospective validation, risk analysis, and formal regulatory review.
\end{itemize}
