...
\begin{figure}[htbp]
  ...
  \textbf{Live inference demonstration screenshot (user-provided).}\\
  The interface shows three reagent-zone crops with per-zone outputs:\\
  Anti-A: Negative, Anti-B: Positive, Anti-D: Negative, and final inferred group: B$^{-}$.\\
  Asset included in the author-provided supplementary visual materials.}}
...
To support reproducible reporting, the following environment metadata is documented with currently available records:
\begin{itemize}
  \item GPU: \textbf{Not explicitly logged in archived training notebook records}
  \item TensorFlow version: \textbf{Not explicitly logged in archived training notebook records}
  \item CUDA/cuDNN version: \textbf{Not explicitly logged in archived training notebook records}
  \item Total training time: \textbf{Not explicitly logged in archived training notebook records}
